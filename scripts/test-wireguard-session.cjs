...
app.whenReady().then(async () => {
  ...
  try {
    ...
    if (current.state !== 'connected' || current.paths.some((route) => route.packetsReceived < 2)) {
      const counts = current.paths.map((route) => `route ${route.route} sent=${route.packetsSent} received=${route.packetsReceived} reachable=${route.reachable} error=${route.lastError || 'none'}`).join(' · ')
      throw new Error(`Persistent WireGuard path health checks did not continue: ${counts}`)
    }
    const summary = current.paths.map((route) => `${route.label}: ${Math.round(route.latencyMs)} ms, ${route.packetsReceived} replies`).join(' · ')
    console.log(`Persistent user-space WireGuard session healthy — ${summary}; relay TUN data ${Math.round(dataPlane.latencyMs)} ms.`)
