...
app.whenReady().then(async () => {
  ...
  const service = new ServiceBridge(process.env.GAMEPATH_SERVICE_PORT ? {
  ...
  try {
    ...
    if (current.state !== 'connected' || !started.dataPlane.reachable) throw new Error('Privileged session did not reach connected state')
    if (started.capture?.state === 'capturing') {
      const ping = spawnSync('ping.exe', ['-n', '1', '-w', '5000', '1.1.1.1'], { encoding: 'utf8', windowsHide: true })
      if (ping.status !== 0) {
        ...
        const addresses = spawnSync('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', `Get-NetIPAddress -AddressFamily IPv4 -InterfaceIndex ${started.capture.adapterIndex} -ErrorAction SilentlyContinue | ConvertTo-Json -Compress`], { encoding: 'utf8', windowsHide: true })
        throw new Error(`Wintun system ping failed; capture=${JSON.stringify(started.capture)}; addresses=${(addresses.stdout || '').trim()}; paths=${JSON.stringify(afterPing.paths)}; ping=${(ping.stdout || ping.stderr || '').trim()}; routes=${(routes.stdout || '').trim()}`)
      }
    }
    console.log(`Privileged multipath session connected ${current.paths.length} paths with ${started.capture?.backend ?? 'no'} ${payload.trafficMode} capture; benchmark loop ${Math.round(started.dataPlane.latencyMs)} ms.`)
  } finally {
