...
function directNodeSelection(enabledTunnels) {
  if (!enabledTunnels.length) {
    return { node: null, error: 'Choose the WireGuard node that will carry your traffic.' }
  }
  if (enabledTunnels.length > 1) {
    return {
      node: null,
      error: `Direct mode sends traffic through one node, but ${enabledTunnels.length} are selected. Choose a single WireGuard node, or switch to relay mode to combine them.`,
    }
  }
  ...
  if (node.kind !== 'wireguard') {
    return {
      node: null,
      error: `${node.name} is a SOCKS5 proxy, which needs a relay to forward to. Choose a WireGuard node for direct mode, or set up a relay.`,
    }
  }
 ...
 * WireGuard node is kept if there is one, and otherwise the first that could
...
function directSelectionAfterSwitch(tunnels) {
  const usable = (tunnel) => tunnel.kind === 'wireguard'
